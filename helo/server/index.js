...
app.post('/api/register', (req, res) => {
  ...
  db.create_user([username, password, profilePic]).then((dbRes) => {
    const { id, username, profilePic } = dbRes[0];
    req.session.userid = username;
    res.status(200).send(id, username, profilePic)
    .catch(() => res.status(500).send('error') ) ;
  });
});
